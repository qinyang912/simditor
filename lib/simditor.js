...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define('simditor', ["jquery","simple-module","simple-hotkeys","simple-uploader","jquery-colpick"], function ($, SimpleModule, simpleHotkeys, simpleUploader) {
    ...
    // like Node.
    module.exports = factory(require("jquery"),require("simple-module"),require("simple-hotkeys"),require("simple-uploader"),require("jquery-colpick"));
  } else {
    root['Simditor'] = factory(jQuery,SimpleModule,simple.hotkeys,simple.uploader);
  }
}(this, function ($, SimpleModule, simpleHotkeys, simpleUploader) {

var AlignmentButton, Attach, AttachButton, BackgroundColorButton, BlockquoteButton, BoldButton, Button, CheckBox, CheckboxButton, ClearFormatButton, Clipboard, CodeButton, CodePopover, ColorButton, CommandBase, CommandUtil, Consolidator, DomRange, DomRangeMemento, DomTreeExtractor, FontFamilyButton, FontScaleButton, FormatPaintButton, FormatPainterCommand, Formatter, FragmentContainer, FragmentsCondition, GlobalLink, HrButton, ImageBlock, ImageButton, IndentButton, Indentation, InfoArea, InlineCommand, InputManager, ItalicButton, Keystroke, LineHeightButton, LinkButton, LinkPopover, ListButton, NodeComparer, OrderListButton, OutdentButton, Popover, RangeFragmentsTraverser, RangeIterator, Selection, Simditor, StrikethroughButton, StripCommand, StripElementCommand, TableButton, TaskBlock, TimeStampButton, TitleButton, Toolbar, TypeColorButton, UnSelectionBlock, UnderlineButton, UndoButton, UndoManager, UnorderListButton, Util, WordNum,
...
Formatter = (function(superClass) {
  ...
  Formatter.prototype._init = function() {
    ...
    this._allowedAttributes = $.extend({
      img: ['src', 'alt', 'width', 'height', 'data-non-image', 'data-bucket', 'data-key-name', 'data-osskey', 'data-name', 'class', 'data-location'],
      a: ['href', 'target', 'class', 'title'],
      font: ['color', 'size', 'face', 'style'],
      code: ['class'],
      p: ['class', 'data-unique-id', 'data-file-id', 'data-file-name', 'data-file-src', 'data-attach', 'data-img', 'data-global-link', 'data-setting', 'data-task-block', 'data-map', 'data-record'],
      span: ['class', 'contenteditable', 'data-name', 'data-size', 'href', 'data-bucket', 'data-osskey', 'data-key-name', 'title', 'data-global-link-type', 'data-title', 'data-sub-title', 'data-attach-type', 'data-record-control', 'data-duration'],
  ...
  Formatter.prototype.cleanNode = function(node, recursive) {
    var $blockEls, $childImg, $node, $p, $td, allowedAttributes, attr, contents, fileId, isDecoration, len, len1, n, ref, ref1, text, textNode, w, z;
    $node = $(node);
    if (!($node.length > 0)) {
      return;
    }
...
Keystroke = (function(superClass) {
  ...
  Keystroke.prototype._initKeystrokeHandlers = function() {
    ...
    this.add('backspace', '*', (function(_this) {
      return function(e) {
        var $blockEl, $prevBlockEl, $rootBlock, $unSelectionWrapper, indentationResult, isWebkit;
        try {
          $rootBlock = _this.editor.selection.rootNodes().first();
        } catch (_error) {
          return false;
        }
        indentationResult = _this.editor.indentation.backspace(e);
        if (indentationResult) {
          return true;
        }
    ...
    this.add('backspace', 'li', (function(_this) {
      return function(e, $node) {
        var $br, $childList, $newLi, $prevChildList, $prevNode, $textNode, isFF, listEl, newBlockEl, newListEl, range, text;
        $childList = $node.children('ul, ol');
        $prevNode = $node.prev('li');
        if (_this.editor.util.isEmptyNode($node) && $prevNode.length === 0) {
          listEl = $node.parent();
          newBlockEl = $('<p/>').append(_this.editor.util.phBr).insertAfter(listEl);
...
Util = (function(superClass) {
  ...
  Util.prototype.browser = (function() {
    var chrome, edge, electron, firefox, ie, ref, ref1, ref2, ref3, ref4, safari, ua;
  ...
  Util.prototype.replaceHotkey = function(h) {
    if (this.os.mac) {
...
Indentation = (function(superClass) {
  ...
  Indentation.prototype.backspace = function(e) {
    var $blockEl, codeButton, range;
    codeButton = this.editor.toolbar.findButton('code');
    if (!(this.opts.tabIndent || (codeButton && codeButton.active))) {
      return;
    }
    range = this.editor.selection.range();
    if (!(range && range.collapsed)) {
...
UnSelectionBlock = (function(superClass) {
  ...
  UnSelectionBlock._tpl = {
    ...
    attach: "<inherit> <span class='" + UnSelectionBlock.className.inlineWrapper + "'> <span class='" + UnSelectionBlock.className.attach + " " + UnSelectionBlock.className.content + "' contenteditable='false'> <span data-attach-type=''></span> <span data-name=''></span> <span data-size='24M'></span> <span class='unselection-attach-operation' contenteditable='false'> <span class='simditor-r-icon-arrow_down unselection-attach-operation-icon unselection-attach-more' title='更多'> <span class='unselection-attach-menu'> <a class='unselection-attach-menu-item unselection-attach-download' title='下载文件' target='_blank'></a> <span class='unselection-attach-menu-item unselection-attach-delete' title='删除文件'></span> </span> </span> </span> </span> </span> </inherit>",
    img: "<img src='' alt=''>",
  ...
  UnSelectionBlock.prototype._init = function() {
    ...
    this.editor.body.on('click.simditor-unSelection', "[data-map] .wrap img,[data-map] .wrap", (function(_this) {
      return function(e) {
        return _this._unMapClick(e);
      };
    })(this));
    $(document).on('click.simditor-unSelection-' + this.editor.id, (function(_this) {
      return function(e) {
        if (!_this._isUnSelectionClick && !_this.editor.imageBlock.isResize) {
  ...
  UnSelectionBlock.fillDataToAttach = function(data, wrapper) {
    var $name, $size, $type, contentType, type;
    ...
    if (data && data.file) {
      ...
      return $type.attr('data-attach-type', type);
    }
  };
  ...
  UnSelectionBlock.prototype._selectCurrent = function(type) {
    ...
    if (this._selectedWrapper) {
      if (type) {
        ...
        return this.editor.trigger(UnSelectionBlock.event.unSelect);
      }
    }
  };
  ...
  UnSelectionBlock.prototype._unMapClick = function(e) {
    var loc, wrapper;
    wrapper = $(e.target).closest("[" + UnSelectionBlock.attr.map + "=true]", this.editor.body);
    if (wrapper.length) {
      loc = wrapper.find('img').attr(UnSelectionBlock.attr.location);
      return this.editor.trigger('selectMap', {
        location: loc
      });
    }
  };
  ...
  UnSelectionBlock.prototype._selectWrapper = function(wrapper) {
    ...
    if (html === '' || html === '<br>') {
      ...
      this._selectCurrent();
      if (wrapper.is("[" + UnSelectionBlock.attr.img + "]") && wrapper.find('img').length) {
        return this._onImageSelect(wrapper.find('img').eq(0));
      }
    }
  };

  UnSelectionBlock.prototype._onImageSelect = function($img) {
    return this.editor.trigger(UnSelectionBlock.event.select, $img);
  };
...
WordNum = (function(superClass) {
  ...
  WordNum.blockNodes = ["div", "p", "ul", "ol", "li", "blockquote", "hr", "pre", "h1", "h2", "h3", "h4", "h5", "table", "td", "th"];
...
CheckBox = (function(superClass) {
  ...
  CheckBox.prototype._init = function() {
    this.editor = this._module;
    this.editor.body.on('click.simditor-check-box-item', '.check-box-item-unchecked,.check-box-item-checked', (function(_this) {
      return function(e) {
        return _this._detect2(e);
      };
    })(this));
    return $(document).on('keydown.simditor-check-box-item' + this.editor.id, (function(_this) {
      return function(e) {
        switch (e.which) {
          case 13:
            return _this._emptyLineDetect(e);
          case 8:
            return _this._emptyLineDetect(e);
        }
      };
  ...
  CheckBox.prototype._emptyLineDetect = function(e) {
    var $root, roots;
    roots = this.editor.selection.rootNodes();
    if (roots && roots.length) {
      $root = $(roots[0]);
      if ($root.is(CheckBox.selector.checkbox) && $.trim($root.text()) === '') {
        this.editor.selection.save();
...
ImageBlock = (function(superClass) {
  ...
  function ImageBlock() {
    return ImageBlock.__super__.constructor.apply(this, arguments);
  }

  ImageBlock.pluginName = 'ImageBlock';

  ImageBlock.prototype._currentResizeImg = null;

  ImageBlock.prototype.isResize = false;

  ImageBlock.prototype._init = function() {
    this.editor = this._module;
    this._initResizeBar();
  ...
  ImageBlock.prototype._setPosition = function() {
    var $img, editorOffset, height, imgH, imgOffset, left, top, width;
    $img = this._currentResizeImg;
    ...
    return this._resize.css({
      top: top + height - 8,
      left: left + width - 8
    });
  };

  ImageBlock.prototype._initResizeBar = function() {
    var tpl;
    tpl = "<div class=\"simditor-resize-bar\" title=\"拖动缩放\"></div>";
    this._resize = $(tpl);
    this.editor.el.append(this._resize);
    this._resize.on('mousedown', (function(_this) {
      return function(e) {
        _this.isResize = true;
        _this.startX = e.clientX;
        _this.startY = e.clientY;
        _this.editor.el.on('click.image-resize', function() {
          _this._offResize();
          return false;
        });
        $(document).on('mouseup.image-resize', function() {
          _this._offResize();
          return false;
        });
        $(document).on('mousemove.image-resize', function(e) {
          if (e.clientX !== _this.startX || e.clientY !== _this.startY) {
            _this._setSize({
              x: e.clientX - _this.startX,
              y: e.clientY - _this.startY
            });
            _this.startX = e.clientX;
            _this.startY = e.clientY;
          }
    ...
    return this._resize.on('click', (function(_this) {
      return function() {
        return false;
      };
    })(this));
  ...
  ImageBlock.prototype._offResize = function() {
    $(document).off('mouseup.image-resize');
    $(document).off('mousemove.image-resize');
    this.editor.el.off('click.image-resize');
    return setTimeout((function(_this) {
      return function() {
        return _this.isResize = false;
      };
    })(this), 0);
  };
...
Attach = (function(superClass) {
  extend(Attach, superClass);

  function Attach() {
    return Attach.__super__.constructor.apply(this, arguments);
  }

  Attach.pluginName = 'Attach';

  Attach.prototype._init = function() {
    var uploadOpts;
    this.editor = this._module;
  ...
  Attach.prototype._initUploader = function() {
    var uploadProgress;
  ...
  Attach.prototype.createAttach = function(file) {
  ...
  Attach.prototype.upload = function(file, opt) {
    if (!this.uploader) {
      return;
    }
    return this.uploader.upload(file, opt);
  };
...
CommandUtil = {
  ...
  offsetParent: function(_child, _parent) {
    var $child, $parent, offsetX, offsetY;
    offsetX = 0;
    offsetY = 0;
    $child = $(_child);
    $parent = $(_parent);
...
Button = (function(superClass) {
  ...
  Button.prototype._init = function() {
    ...
    if (this.shortcut != null) {
      this.editor.hotkeys.add(this.editor.util.replaceHotkey(this.shortcut), (function(_this) {
        return function(e) {
          _this.el.mousedown();
          return false;
        };
  ...
  Button.prototype.renderMenu = function() {
    var $menuBtnEl, $menuItemEl, len, menuItem, param, ref, ref1, results, w;
      ...
      if (menuItem.shortcut) {
        ...
        results.push((function(_this) {
          return function(param) {
            return _this.editor.hotkeys.add(_this.editor.util.replaceHotkey(menuItem.shortcut), function(e) {
              if (!_this.disabled) {
                _this.command(param);
                return false;
              }
  ...
  TitleButton.prototype._init = function() {
    ...
    this.menu.forEach((function(_this) {
      return function(m) {
        ...
        _t = _this.editor.util.replaceHotkey(m.shortcut);
        if (_this.editor.util.os.mac) {
          _t = _t.replace('alt', 'opt');
        }
        return m.title = m.text + (" (" + _t + ")");
      };
    })(this));
...
ColorButton = (function(superClass) {
  ...
  ColorButton.prototype.renderMenu = function() {
    ...
    customColor = "<label class=\"custom-color\">" + (this._t('customColor')) + "</label>";
    $(list + customColor).appendTo(this.menuWrapper);
    this.menuWrapper.on('mousedown', '.color-list,.custom-color,input', (function(_this) {
      return function(e) {
        return false;
      };
    })(this));
    this.menuWrapper.on('click', '.font-color', (function(_this) {
      return function(e) {
        ...
        return _this._format(hex);
      };
    })(this));
    return this.menuWrapper.find('.custom-color').colpick({
      layout: 'hex',
      submit: 0,
      onChange: (function(_this) {
        return function(hsb, hex) {
          return _this._format("#" + hex);
        };
      })(this)
    });
...
TypeColorButton = (function(superClass) {
  extend(TypeColorButton, superClass);

  function TypeColorButton() {
    return TypeColorButton.__super__.constructor.apply(this, arguments);
  }

  TypeColorButton.prototype.name = 'color';

  TypeColorButton.prototype.icon = 'type-color';
...
BlockquoteButton = (function(superClass) {
  ...
  BlockquoteButton.prototype.shortcut = 'super+alt+.';

  BlockquoteButton.prototype._init = function() {
    if (this.editor.util.os.mac) {
      this.title = this.title + " ( Cmd + opt + . )";
    } else {
      this.title = this.title + " ( Ctrl + alt + ' )";
      this.shortcut = 'super+alt+\'';
    }
    return BlockquoteButton.__super__._init.call(this);
  };
...
CodeButton = (function(superClass) {
  ...
  CodeButton.prototype.shortcut = 'super+alt+v';
  ...
  CodeButton.prototype.render = function() {
    ...
    args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    if (this.editor.util.os.mac) {
      this.title = this.title + ' ( Cmd + opt + v )';
    } else {
      this.title = this.title + ' ( Ctrl + alt + v )';
    }
...
LinkButton = (function(superClass) {
  ...
  LinkButton.prototype.shortcut = 'super+alt+k';
  ...
  LinkButton.prototype.render = function() {
    ...
    args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    if (this.editor.util.os.mac) {
      this.title = this.title + ' ( Cmd + opt + k )';
    } else {
      this.title = this.title + ' ( Ctrl + alt + k )';
    }
  ...
  ImageButton.prototype._initUploader = function($uploadItem) {
    ...
    $uploadItem.on('change', 'input[type=file]', (function(_this) {
      return function(e) {
        ...
        }, _this.editor.util.browser.electron ? 500 : 0);
    ...
    this.editor.uploader.on('uploadsuccess', (function(_this) {
      return function(e, file, result) {
        ...
        if (result.success === false) {
          ...
          alert(msg);
          return img_path = _this.defaultImage;
        } else {
          if (result.ALY === true) {
            return $.ajax({
              url: _this.editor.opts.upload.GET_FILE_FROM_ALI,
...
HrButton = (function(superClass) {
  ...
  HrButton.prototype.shortcut = 'super+alt+h';

  HrButton.prototype.render = function() {
    if (this.editor.util.os.mac) {
      this.title = this.title + ' ( Cmd + opt + h )';
    } else {
      this.title = this.title + ' ( Ctrl + alt + h )';
    }
    return HrButton.__super__.render.call(this);
  };
...
TableButton = (function(superClass) {
  ...
  TableButton.prototype._init = function() {
    ...
    this.editor.keystroke.add('down', 'th', (function(_this) {
      return function(e, $node) {
        _this._tdNav($node, 'down');
        return true;
      };
    })(this));
    this.editor.body.on('contextmenu', '.simditor-table table', (function(_this) {
      return function(e) {
        e.preventDefault();
        return setTimeout(function() {
          var offset;
          offset = Simditor.CommandUtil.offsetParent(e.target, _this.editor.el);
    ...
    this.editor.on('selectionchanged.table-contextmenu', (function(_this) {
      return function(e) {
        return _this.hideContextMenu();
      };
    })(this));
  ...
  TableButton.prototype._editTableTpl = function() {
    var tpl;
    return tpl = "<div class=\"menu-edit-table\">\n  <ul>\n    <li>\n      <a tabindex=\"-1\" unselectable=\"on\" class=\"menu-item\"\n        href=\"javascript:;\" data-param=\"deleteRow\">\n        <span>" + (this._t('deleteRow')) + "</span>\n      </a>\n    </li>\n    <li>\n      <a tabindex=\"-1\" unselectable=\"on\" class=\"menu-item\"\n        href=\"javascript:;\" data-param=\"insertRowAbove\">\n        <span>" + (this._t('insertRowAbove')) + "</span>\n      </a>\n    </li>\n    <li>\n      <a tabindex=\"-1\" unselectable=\"on\" class=\"menu-item\"\n        href=\"javascript:;\" data-param=\"insertRowBelow\">\n        <span>" + (this._t('insertRowBelow')) + "</span>\n      </a>\n    </li>\n    <li><span class=\"separator\"></span></li>\n    <li>\n      <a tabindex=\"-1\" unselectable=\"on\" class=\"menu-item\"\n        href=\"javascript:;\" data-param=\"deleteCol\">\n        <span>" + (this._t('deleteColumn')) + "</span>\n      </a>\n    </li>\n    <li>\n      <a tabindex=\"-1\" unselectable=\"on\" class=\"menu-item\"\n        href=\"javascript:;\" data-param=\"insertColLeft\">\n        <span>" + (this._t('insertColumnLeft')) + "</span>\n      </a>\n    </li>\n    <li>\n      <a tabindex=\"-1\" unselectable=\"on\" class=\"menu-item\"\n        href=\"javascript:;\" data-param=\"insertColRight\">\n        <span>" + (this._t('insertColumnRight')) + "</span>\n      </a>\n    </li>\n    <li><span class=\"separator\"></span></li>\n    <li>\n      <a tabindex=\"-1\" unselectable=\"on\" class=\"menu-item\"\n        href=\"javascript:;\" data-param=\"deleteTable\">\n        <span>" + (this._t('deleteTable')) + "</span>\n      </a>\n    </li>\n  </ul>\n</div>";
  };

  TableButton.prototype.renderMenu = function() {
    var $table;
    $("<div class=\"menu-create-table\">\n</div>\n" + (this._editTableTpl())).appendTo(this.menuWrapper);
  ...
  TableButton.prototype.renderContextMenu = function() {
    this.contextmenu = $('<div class="simditor-table-context-menu"></div>');
    this.contextmenu.append(this._editTableTpl());
    this.editor.el.append(this.contextmenu);
    this.contextmenu.on('mousedown', (function(_this) {
      return function(e) {
        return false;
      };
    })(this));
    return this.contextmenu.on('click', 'a.menu-item', (function(_this) {
      return function(e) {
        var btn, param;
        btn = $(e.currentTarget);
        param = btn.attr('data-param');
        if (_this.contextMenuUnit && _this.contextMenuUnit.length) {
          _this.command(param, _this.contextMenuUnit);
        }
        return _this.hideContextMenu();
      };
    })(this));
  ...
  TableButton.prototype.showContextMenu = function(opt, target) {
    this.contextmenu.css({
      top: opt.top,
      left: opt.left
    });
    this.contextMenuUnit = $(target).closest('td, th');
    if (this.contextMenuUnit && this.contextMenuUnit.length) {
      return this.contextmenu.show();
    } else {
      return this.contextMenuUnit = null;
    }
  };

  TableButton.prototype.hideContextMenu = function() {
  ...
  TableButton.prototype.command = function(param, _td) {
    var $td;
    $td = _td ? _td : $(this.editor.selection.containerNode().closest('td, th'));
    if (!($td.length > 0)) {
      return;
    }
    if (param === 'deleteRow') {
...
StrikethroughButton = (function(superClass) {
  ...
  StrikethroughButton.prototype.shortcut = 'super+shift+s';

  StrikethroughButton.prototype.render = function() {
    if (this.editor.util.os.mac) {
      this.title = this.title + ' ( Cmd + shift + s )';
    } else {
      this.title = this.title + ' ( Ctrl + shift + s )';
    }
    return StrikethroughButton.__super__.render.call(this);
  };
...
AlignmentButton = (function(superClass) {
  ...
  AlignmentButton.prototype.shortcutList = ['super+alt+y', 'left', 'center', 'right'];
  ...
  AlignmentButton.prototype._init = function() {
    ...
    AlignmentButton.__super__._init.call(this);
    return this.editor.hotkeys.add(this.editor.util.replaceHotkey(this.shortcutList[0]), (function(_this) {
      return function(e) {
        var _i, c, i;
        if (!_this.disabled) {
          console.log('do align');
          c = _this.nodes.first().css('text-align');
          console.log(c);
          i = _this.shortcutList.indexOf(c);
          if (i === -1) {
            i = 1;
          }
          console.log('i', i);
          ...
          return _this.command(_this.shortcutList[_i]);
        }
      };
    })(this));
  };
  ...
  AlignmentButton.prototype.render = function() {
    if (this.editor.util.os.mac) {
      this.title = this.title + ' ( Cmd + opt + y )';
    } else {
      this.title = this.title + ' ( Ctrl + alt + y )';
    }
    return AlignmentButton.__super__.render.call(this);
  };
...
AttachButton = (function(superClass) {
  ...
  AttachButton.prototype._initUploader = function() {
    ...
    return this.el.on('change', 'input[type=file]', (function(_this) {
      return function(e) {
        if (_this.editor.inputManager.focused) {
          _this.editor.attach.upload(_this.input, {
            inline: true
          });
...
TimeStampButton = (function(superClass) {
  ...
  TimeStampButton.prototype.shortcut = 'super+alt+t';

  TimeStampButton.prototype.render = function() {
    if (this.editor.util.os.mac) {
      this.title = this.title + ' ( Cmd + opt + t )';
    } else {
      this.title = this.title + ' ( Ctrl + alt + t )';
    }
    return TimeStampButton.__super__.render.call(this);
  };
...
LineHeightButton = (function(superClass) {
  ...
  LineHeightButton.prototype.name = 'line-height';

  LineHeightButton.prototype.icon = 'line-height';
  ...
  LineHeightButton.prototype.command = function(param) {
    ...
    rootNodes.each((function(_this) {
      return function(i) {
        var $node;
        $node = rootNodes.eq(i);
        return $node.css('line-height', param);
      };
    })(this));
    return this.editor.trigger('valuechanged');
  };
...
ClearFormatButton = (function(superClass) {
  extend(ClearFormatButton, superClass);

  function ClearFormatButton() {
    return ClearFormatButton.__super__.constructor.apply(this, arguments);
  }

  ClearFormatButton.prototype.name = 'clear-format';

  ClearFormatButton.prototype.icon = 'format-clear';

  ClearFormatButton.prototype.command = function() {
...
CheckboxButton = (function(superClass) {
  ...
  CheckboxButton.prototype.render = function() {
    ...
    return CheckboxButton.__super__.render.call(this);
  };
